...
* **快速发送**：使用 `/to <别名> <内容>` 来快速发送临时的**纯文本**消息，不影响当前锁定的模式。
...
2.  **`ADMIN_USER_ID` (您的管理员ID)**
    * 去 Telegram 搜索 **`@userinfobot`**。
    * 给它发送任意消息，它会回复您的个人信息。
    * 找到 `Id:` 后面那串**纯数字**（例如 `12345678`）。**复制它**。

### ⚙️ 阶段二：GitHub 自动构建镜像

1.  确保您的 GitHub 仓库中包含 `bot.py`, `requirements.txt`, `Dockerfile` 和 `.github/workflows/build-docker.yml` 这四个文件。
2.  将代码推送到 `main` 分支，GitHub Actions 会**自动开始**构建 Docker 镜像。
3.  在 GitHub 仓库主页，点击 **"Actions"** 标签页，等待工作流出现**绿色对勾 ✅**。
4.  回到仓库主页，点击右侧的 **"Packages"**，找到您刚构建的镜像。
5.  点击镜像名称，进入 "Package settings" (包设置)，将可见性 (Visibility) 修改为 **"Public" (公开)**。
6.  **复制**该页面的镜像路径，它看起来像：`ghcr.io/您的用户名/您的仓库名:latest`。

### 📡 阶段三：Leaflow 首次部署 (获取群组ID)

1.  登录 Leaflow，创建一个新服务。
2.  选择 **“从 Docker 镜像部署”**。
3.  在“镜像名称”处，**粘贴**您在阶段二复制的镜像路径（例如 `ghcr.io/username/repo:latest`）。
4.  找到 **“环境变量”** (Environment Variables) 部分，添加**两个**变量：
    * `BOT_TOKEN`: (粘贴您在阶段一获取的机器人令牌)
    * `ADMIN_USER_ID`: (粘贴您在阶段一获取的您的个人ID)
5.  点击 **“部署”**。
6.  等待服务变为 "Running" (运行中)。
7.  把您的机器人**拉进所有**您想转发消息的目标群组（例如 `g1` 群和 `g2` 群）。
8.  在 `g1` 群里，发送命令 `/getid`。机器人会回复 `g1` 群的 ID（例如 `-100123456`）。**复制这个ID**。
9.  在 `g2` 群里，发送命令 `/getid`。机器人会回复 `g2` 群的 ID。**复制这个ID**。

### 🛰️ 阶段四：Leaflow 最终部署 (配置群组)

1.  回到 Leaflow 您的服务设置中，找到 **“环境变量”**。
2.  添加**第三个**，也是最后一个变量：
    * `GROUP_MAP`: (填入您规划好的“别名地图”)
    * **格式**：`别名1:ID1,别名2:ID2,别名3:ID3`
    * **示例**：`g1:-100123456,g2:-100789012`
3.  保存环境变量，然后点击 **"Redeploy" (重新部署)**。
...
2.  **发送 `/set g1`** (设置模式为 g1)
    * **机器人回复**：“✅ 已切换... `g1`”。

3.  **发送一张图片** (可以带说明，也可以不带)
    * **立即查看 `g1` 群**：机器人（用您伪装的名字）在群里发了这张图片。

4.  **发送纯文本** `这是g1的测试`
    * **立即查看 `g1` 群**：机器人发了 `这是g1的测试`。

5.  **发送 `/set g2`** (切换模式到 g2)
    * **机器人回复**：“✅ 已切换... `g2`”。
...
8.  **发送 `/to g1 这是一条给g1的临时文本`** (临时快速发送)
    * **立即查看 `g1` 群**：机器人发了 `这是一条给g1的临时文本`。

9.  **再次发送 `/who`**
    * **机器人回复**：“ℹ️ 当前消息目标群组是: `g2`”。 (证明 `/to` 命令**不会**改变您用 `/set` 设定的模式)。

10. **发送 `/set none`** (取消所有模式)
    * **机器人回复**：“✅ 模式已取消。”
