...
function handleRenderVerifyPage(userId, siteKey) {
  ...
  <head>
    ...
    <style>
      body { font-family: -apple-system, sans-serif; display: flex; flex-direction: column; justify-content: center; align-items: center; height: 100vh; background-color: #f5f5f7; margin: 0; }
      .card { background: white; padding: 30px; border-radius: 12px; box-shadow: 0 4px 20px rgba(0,0,0,0.08); text-align: center; max-width: 90%; width: 350px; }
      h2 { color: #1d1d1f; margin-bottom: 10px; font-size: 22px; }
      p { color: #86868b; font-size: 14px; margin-bottom: 25px; line-height: 1.4; }
  ...
  <body>
    <div class="card">
      <h2>🔐 安全验证</h2>
      <p>为了防止广告群发机骚扰，请完成下方的 Cloudflare 人类身份验证。</p>
      ...
      function onVerify(token) {
        ...
        .then(data => {
          if (data.success) {
            ...
            statusDiv.innerText = '🎉 验证成功！请返回 Telegram 聊天窗口直接发消息。';
          } else {
            statusDiv.className = 'error';
            statusDiv.innerText = '❌ 验证失败：' + data.msg;
          }
        })
        .catch(() => {
          statusDiv.className = 'error';
          statusDiv.innerText = '❌ 网络请求失败，请刷新重试。';
        });
      }
    </script>
  </body>
...
async function handleVerifyToken(request, userId, env) {
  try {
    ...
    if (!token) return new Response(JSON.stringify({ success: false, msg: 'Token 缺失' }));
    ...
    if (outcome.success) {
      ...
      return new Response(JSON.stringify({ success: false, msg: '验证未通过' }));
    }
  } catch (e) {
  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname.startsWith('/verify/')) {
      ...
      if (request.method === 'GET') {
        return handleRenderVerifyPage(userId, env.CF_TURNSTILE_SITEKEY);
      } else if (request.method === 'POST') {
        return handleVerifyToken(request, userId, env);
      }
    }
...
async function handleAdminBaseConfigMenu(chatId, messageId, env) {
  ...
  const currentMode = "Cloudflare Turnstile 验证";
  const menuText = `
⚙️ <b>基础配置 (验证设置)</b>

<b>当前验证模式:</b> 🌐 ${currentMode}
